...
def sample_hh_like(hours: pd.DatetimeIndex, fit: Dict, rng: np.random.Generator) -> np.ndarray:
    """Return hourly kWh array for one synthetic HH-like actor (also used for prosumer load)."""
    ...
    # Draw lnD per day by cluster of day (use mode cluster of day)
...
def sample_pv(hours: pd.DatetimeIndex, pv_fit: Dict, kWp: float, rng: np.random.Generator) -> np.ndarray:
    ...
    # Build daily M_t via LL and day-state via MK
    day_state = {}
    M_day = {}
    prev_state = {}
    for d in uniq_dates:
        ...
        if s not in prev_state:
            # equilibrium clear prob
            p01 = P[0,1]; p10 = P[1,0]
            pi_clear = p10/(p01+p10+1e-9)
            stt = 1 if rng.uniform() < pi_clear else 0
        else:
            stt = 1 if rng.uniform() < P[prev_state[s], 1] else 0
        prev_state[s] = stt
        day_state[d] = stt
...
def equal_level_fill(supply: float, demands: np.ndarray) -> (np.ndarray, float):
    """Water-filling: split evenly across positive residuals; cap by demand."""
    n = len(demands)
    if n == 0 or supply <= 0:
...
def run_monte_carlo(
    ...
    zonal: pd.DataFrame, pun: pd.DataFrame,
    price_layer, S: int, seed: int
) -> Dict[str, Any]:

    rng = np.random.default_rng(seed)
    # Prebuild price frame aligned to hours
    z = zonal.set_index("timestamp").reindex(hours)["zonal_price (EUR_per_MWh)"].ffill()
    p = pun.set_index("timestamp").reindex(hours)["PUN (EUR_per_MWh)"].ffill()
    price_df = pd.DataFrame({"timestamp": hours, "zonal": z.values, "pun": p.values})

    nP = len(prosumer_ids); nH = len(hh_ids); nK = len(shop_ids)

    # Storage for scenario summaries
    ...
    platform_gap = np.zeros(S); platform_fees = np.zeros(S); platform_fixed = np.zeros(S)
    pv_gen = np.zeros(S); cons_total = np.zeros(S)
    ...
    for sidx in range(S):
        ...
        for _, row in price_df.iterrows():
            ts = row["timestamp"]; zon = float(row["zonal"]); punv = float(row["pun"])
            layer = price_layer(zon, punv)
            # Unmet demands
            U_hh = np.array([hh_load[h][ts] for h in hh_ids], dtype=float)
            U_sh = np.array([shop_load[k][ts] for k in shop_ids], dtype=float)

            # Baseline retail-only cost (for savings)
            ...
            cons_sum += ( (m_hh + imp_hh) + (m_shop + imp_shop) )  # running tally ok

        # Platform fees & fixed
        fees = 12.0 * (nP * layer_fee("pros") + nH * layer_fee("hh") + nK * layer_fee("shop"))
        # But our fees are not from layer; pass via closure? Simplify by returning zero here; we put real below
        # Update after loop using inputs outside
    ...
    # Fees & fixed cost (constant across scenarios)
    fees = 12.0 * (len(prosumer_ids) * price_params["f_pros"] + len(hh_ids) * price_params["f_hh"] + len(shop_ids) * price_params["f_shop"])
...
# The above needs access to fee parameters; provide a setter via closure in app.
# To keep the signature clean, we inject them by module-level variable just-in-time:
price_params = {"f_pros": 0.0, "f_hh": 0.0, "f_shop": 0.0, "platform_fixed": 0.0}
def set_fee_params(f_pros, f_hh, f_shop, platform_fixed):
    price_params.update({"f_pros": f_pros, "f_hh": f_hh, "f_shop": f_shop, "platform_fixed": platform_fixed})
